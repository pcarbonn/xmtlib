...
(pop)
****** Database Error: a JOIN clause is required before ON
(push)
(assert (forall ((x Node)) (Edge x x)))
(pop)
(assert false)
...
Groundings:
 ...
 - (Edge x x):
    T: SELECT Edge_TU_10.a_1 AS x, "true" AS G FROM Edge_TU AS Edge_TU_10 WHERE Edge_TU_10.a_1 = Edge_TU_10.a_0
    F: SELECT Edge_UF_10.a_1 AS x, "false" AS G FROM Edge_UF AS Edge_UF_10 WHERE Edge_UF_10.a_1 = Edge_UF_10.a_0
    G : SELECT Edge_G_10.a_1 AS x, Edge_G_10.G AS G FROM Edge_G AS Edge_G_10 WHERE Edge_G_10.a_1 = Edge_G_10.a_0
 - (forall ((x Node)) (Edge x x)):
    TU: SELECT and_aggregate(G) as G from (SELECT Edge_G_10.a_1 AS x, Edge_G_10.G AS G FROM Edge_G AS Edge_G_10 WHERE Edge_G_10.a_1 = Edge_G_10.a_0) HAVING and_aggregate(G) <> "false"
    UF: SELECT G as G from (SELECT Edge_UF_10.a_1 AS x, "false" AS G FROM Edge_UF AS Edge_UF_10 WHERE Edge_UF_10.a_1 = Edge_UF_10.a_0)
    G : SELECT and_aggregate(G) as G from (SELECT Edge_G_10.a_1 AS x, Edge_G_10.G AS G FROM Edge_G AS Edge_G_10 WHERE Edge_G_10.a_1 = Edge_G_10.a_0)
