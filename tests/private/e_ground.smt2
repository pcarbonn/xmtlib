...
(assert (is_true true))
(pop)
****** Database Error: unrecognized token: "{" in CREATE VIEW IF NOT EXISTS is_true_4 AS SELECT if_, G FROM (SELECT (eq_({terms})) AS if_, "true" AS G FROM is_true_TU AS is_true_TU_4 WHERE "true" = is_true_TU_4.a_0) at offset 71
(push)
(assert (not (is_true (exists ((x Bool)) (is_true x)))))
(pop)
(assert false)
